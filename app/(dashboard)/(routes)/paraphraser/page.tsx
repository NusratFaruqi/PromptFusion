...
import { FileType2, MessageSquare, Send } from "lucide-react";
...
const ConversationPage = () => {
  ...
  const onSubmit = async (values: z.infer<typeof formSchema>) => {
    try {
      ...
      const response = await axios.post("/api/paraphraser", {
  ...
  return (
    <div className="bg-gradient-to-r from-slate-700 via-stone-900 to-slate-800 h-full py-4 relative">
      <Heading
        ...
        description="Most Effective Text Paraphraser"
        icon={FileType2}
      ...
      <div className="px-4 lg:px-8">
        <div className="space-y-4 mt-4">
          ...
          {messages.length === 0 && !isLoading && (
            <Empty label="No Text was Paraphrased." />
          )}
                ...
                <FormItem className="col-span-12 lg:col-span-10">
                  <FormControl className="m-0 p-0">
                    <Input
                      ...
                      disabled={isLoading}
                      placeholder="Enter your text for paraphrasing magic"
                      {...field}
                    />
                  </FormControl>
                </FormItem>
